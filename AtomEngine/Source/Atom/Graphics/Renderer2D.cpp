...
namespace atom
{
	Renderer2D::Renderer2D()
	{
		...
		glBufferData(GL_ARRAY_BUFFER, RENDERER2D_BUFFER_SIZE, nullptr, GL_DYNAMIC_DRAW);
		...
		glVertexAttribPointer(SHADER_VERTEX_INDEX, 3, GL_FLOAT, GL_FALSE, RENDERER2D_VERTEX_SIZE, (const void*)0);
		glVertexAttribPointer(SHADER_TEXCOORD_INDEX, 2, GL_FLOAT, GL_FALSE, RENDERER2D_VERTEX_SIZE, (const void*)offsetof(VertexData, VertexData::texCoord));
		glVertexAttribPointer(SHADER_TEXID_INDEX, 1, GL_FLOAT, GL_FALSE, RENDERER2D_VERTEX_SIZE, (const void*)offsetof(VertexData, VertexData::texID));
		glVertexAttribPointer(SHADER_COLOR_INDEX, 4, GL_UNSIGNED_BYTE, GL_TRUE, RENDERER2D_VERTEX_SIZE, (const void*)offsetof(VertexData, VertexData::color));
		...
		Debug::Log("Renderer2D initialized\n");
	}
	...
	Renderer2D::~Renderer2D()
	{
		...
		Debug::Log("Renderer2D destroyed\n");
	}
	...
	void Renderer2D::DrawString(const Label* label)
	{
		...
		*/
		...
		const float scaleX = 1024.0f / 32.0f;
		const float scaleY = 576.0f / 18.0f;
